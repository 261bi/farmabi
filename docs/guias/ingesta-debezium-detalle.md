...
El archivo `compose.yml` levanta estos servicios:
...
- nombre del cluster: `farmabi-debezium`
...
Get-Content .\4_cargar_datos_didacticos_bi.sql -Raw |
  docker exec -i farmabi-oltp-mysql mysql -uroot -proot
```
...
```powershell
docker exec farmabi-debezium-kafka bash -lc "kafka-topics.sh --bootstrap-server kafka:9092 --list"
```
...
Si ya tienes el proyecto `dw-dbt` de `farmabi`, el encaje recomendado queda así:
