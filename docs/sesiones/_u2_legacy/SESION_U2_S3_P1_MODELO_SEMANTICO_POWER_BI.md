...
Nota de uso para la jerarquía `Calendario`:

```text
Calendario -> navegación agregada por año, trimestre, mes y día
dim_fecha[fecha] -> campo suelto solo si necesitas mostrar la fecha exacta
```

No coloques `dim_fecha[fecha]` dentro de la jerarquía `Calendario` para esta práctica. Power BI suele expandir una columna de tipo fecha como jerarquía automática con `Año`, `Trimestre`, `Mes` y `Día`; eso genera niveles duplicados cuando ya tenemos `año`, `trimestre`, `mes_desc` y `dia`.
...
- puedes navegar de año a trimestre, mes y día
- el subtotal de cada nivel se calcula automáticamente
- el modelo responde correctamente desde `dim_fecha` hacia `fact_ventas`
- no aparece una segunda jerarquía interna de `fecha` con `Año`, `Trimestre`, `Mes` y `Día`
