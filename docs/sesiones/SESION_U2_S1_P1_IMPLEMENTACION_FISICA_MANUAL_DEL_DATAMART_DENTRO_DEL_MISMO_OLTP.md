...
- Implementación esperada en el DataMart: columna física `venta_neta` en `fact_ventas`.
...
- Fuente: `pedido_detalles`.
- Implementación esperada en el DataMart: columna física `cantidad_vendida` en `fact_ventas`.
...
- Implementación esperada en el DataMart: columna física `margen_bruto` en `fact_ventas`.

**KPI 4. % Margen bruto**

- Definición: proporción del margen bruto respecto de la venta neta.
- Fuente: derivado desde las medidas de detalle.
- Implementación esperada en el DataMart: columna física `pct_margen_bruto` en `fact_ventas`.

**KPI 5. Ticket promedio por pedido**

- Definición: venta neta promedio por pedido.
- Fuente: `pedidos` + `pedido_detalles`.
- Implementación esperada en el DataMart: indicador derivado.
- Cálculo analítico esperado: `SUM(venta_neta) / COUNT(DISTINCT pedido_id)`.

**KPI 6. Descuento total otorgado**

- Definición: monto total de descuento aplicado en las líneas del pedido.
- Fuente: `pedido_detalles`.
- Implementación esperada en el DataMart: columna física `descuento_total` en `fact_ventas`.
...
- Implementación esperada en el DataMart: columna física `minutos_confirmacion` en `fact_ventas`.
...
- Implementación esperada en el DataMart: columna física `minutos_despacho` en `fact_ventas`.

**KPI 9. Tiempo de entrega (horas)**

- Definición: tiempo entre envío y entrega.
- Base temporal: `fecha_envio -> fecha_entrega` en `pedidos`.
- Implementación esperada en el DataMart: columna física `horas_entrega` en `fact_ventas`.
...
- Implementación esperada en el DataMart: columna física `horas_lead_time` en `fact_ventas`.
...
- Implementación esperada en el DataMart: indicador derivado.
- Cálculo analítico esperado: porcentaje de `pedido_id` cuyo `horas_lead_time` esté dentro del umbral definido por el negocio.

**KPI 12. Cantidad de pedidos**

- Definición: número de pedidos registrados.
- Fuente: `pedidos`.
- Implementación esperada en el DataMart: columna auxiliar `pedido_count` en `fact_ventas`, con valor 1 por fila de hecho.
- Cálculo analítico esperado: `COUNT(DISTINCT pedido_id)` o `SUM(pedido_count)` según el nivel de análisis.
